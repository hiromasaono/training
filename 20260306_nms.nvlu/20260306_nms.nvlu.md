...
---
## PlotsOfData でできること
- 生データと統計量（平均・中央値・箱ひげ図・バイオリン図）を重ねて表示
- 95%信頼区間の描画で推測統計もカバー
- 透明度（alpha）調整で生データと統計量のバランスを自在にコントロール

### 操作フロー
1. 数値データをタブ区切りで貼り付け → 2. プロットタイプを選択 → 3. 調整 → 4. PNG / PDF でダウンロード

- インストール・登録不要。Excelの数値を貼るだけで論文品質の図が作れる
- 統合TV: [PlotsOfDataを使ってデータの統計量とそのグラフを作成する](https://togotv.dbcls.jp/20220314.html)

---
## PlotsOfData — プロットタイプと使い分け
### プロットタイプ
- **Jitter**：個々のデータ点を表示し、分布の全体像を把握
- **Box plot**：四分位・中央値・外れ値の確認
- **Violin plot**：分布の形を直感的に伝えたいとき
- **Beeswarm**：データ点を密度に応じて整列表示
- 生データ + 統計量の重ね表示で透明性の高い図が作れる

### 推測統計
- 95%信頼区間（CI）を表示して条件間の差を視覚的に評価
...
- フローチャート・概念図にも応用可。Google Driveと連携して保存
...
- テキストファイル（1行1要素）を複数アップロードして比較
- **ベン図**（最大6セット）／ **UpSetプロット** ／ **ペアワイズヒートマップ**
- SVG / PDF / PNG でエクスポート
