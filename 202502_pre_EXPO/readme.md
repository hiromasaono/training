# 🧬 卵アレルギーの謎を解く！～バイオインフォマティクス探偵になろう～ 🧬

## 🎓 まずは基礎知識を身につけよう！

### 遺伝子とタンパク質の関係
想像してみてください：
- **遺伝子**は「設計図」
  - 平面的な情報（文字列のような配列）
  - どんなタンパク質を作るかの指示書

- **タンパク質**は「実行部隊」
  - 立体的な構造（3Dの形を持つ）
  - 実際に体の中で働く
  
```
DNAの設計図(遺伝子) → タンパク質(実際に働く分子)
     📝              →      🏗️
    平面              →     立体
```

### アレルギーの仕組み
アレルギーは「鍵と鍵穴」のような関係で起こります：
- アレルゲン（例：オボムコイド）＝鍵🔑
- 体の中の免疫システム＝鍵穴🔒
- 鍵が鍵穴にぴったり合うと、アレルギー反応が起きる！

### 今日の探偵ミッション
卵アレルギーの人の中には、不思議なことに：
- ニワトリの卵 → アレルギー反応が出る😣
- ウズラの卵 → 大丈夫な人もいる😊

**なぜ？** この謎を解くため：
1. オボムコイドという卵の中のタンパク質を調べる
2. 様々な生物の似たタンパク質を比べる
3. その違いとアレルギーの関係を探る

### どうやって調べるの？
1. 世界中の研究者が使うデータベースで配列を探す
2. 配列を比較して違いを見つける
3. 分子系統樹を作って、生物間の関係を調べる

これらの作業を通じて、「どの生物の卵がアレルギーを起こしやすいか」を予測する手がかりを得ることができます！

### 🎯 今日の目標
- 【発見】卵アレルギーの原因となるタンパク質の正体を知る
- 【体験】本物の研究データベースを使ってみる
- 【考察】なぜ鳥の種類によってアレルギー反応が違うのか考える

### ⚡ クイックガイド
- **所要時間**: 約30分
- **難易度**: ★★☆☆☆ (中学生・高校生向け)
- **必要なもの**: インターネットに接続されたPC/タブレット

## 🔍 Step 1: タンパク質データベースで探偵活動を始めよう！

### 🎯 このステップのゴール
- 世界最大のタンパク質データベース「UniProt」の使い方を学ぶ
- アレルギーの原因となるタンパク質を見つける
- 似ているタンパク質を探す方法を知る

### 💡 UniProtって何？
- 世界中の研究者が使う**タンパク質のビッグデータベース**
- なんと約2億5000万個ものタンパク質の情報が登録されている！
- ヒトから細菌まで、様々な生物のデータが無料で見られる

### 🔰 探偵の調査手順

#### 1️⃣ 基本の検索
1. [UniProt](https://www.uniprot.org/)のウェブサイトにアクセス
2. 中央の検索窓に「**ovomucoid**」と入力
   - ⚠️ 英語で入力しましょう！
   - 「Table」表示を選択

#### 2️⃣ 検索結果の見方
検索結果には以下の情報が表示されます：
- **Entry**: タンパク質のID（例：P01005）
- **Entry Name**: タンパク質の略称と生物名
- **Protein Names**: タンパク質の正式名称
- **Organism**: 生物の種名（学名と英名）
- **Length**: アミノ酸の数

#### 3️⃣ BLAST検索で似たタンパク質を探す
1. ニワトリ(Gallus gallus)のオボムコイド(P01005)をクリック
2. 左メニューの「BLAST」をクリック
3. 「Run BLAST」で検索開始
4. 2分ほど待って結果を確認

> 🔍 **BLAST検索結果の読み方**
> - 上位にあるほど、よく似たタンパク質
> - スコアが高いほど類似度が高い
> - 生物種名から進化的な関係も分かる！

### 🌟 チャレンジ：他のアレルゲンも調べてみよう！

以下のアレルゲンも検索してみましょう：

| アレルギー | タンパク質名 | 生物種 |
|------------|--------------|---------|
| 🥛 牛乳 | casein (カゼイン) | Bos taurus (ウシ) |
| 🍞 小麦 | gliadin (グリアジン) | Triticum aestivum (コムギ) |
| 🐟 魚卵 | vitellogenin (ビテロジェニン) | Oncorhynchus keta (シロサケ) |

### 📝 探偵ノート
```
【調べたアレルゲン】: 

【似ているタンパク質を持つ生物 TOP3】
1. 
2. 
3. 

【気づいたこと】:

```

> 💡 **探偵のコツ**
> - 分からない英語は Google翻訳やDeepLを使おう
> - 生物種の和名はGoogleで調べてみよう
> - 結果の見方が分からないときは、スタッフに相談しよう！

> 💡 **研究者のコツ**  
> FASTAって何？ → タンパク質の「設計図」みたいなもの。
> アルファベット1文字が1つのアミノ酸を表しています！

## 🔄 Step 2: タンパク質配列を並べて謎を解こう！

### 🎯 このステップのゴール
- 配列アラインメント（配列並べ）の基本を理解する
- タンパク質の進化の痕跡を見つける
- 配列の違いからアレルギーの謎に迫る

### 💡 アラインメントって何？
配列アラインメントは、まるで「タンパク質のパズル」です！
```
例えば、こんな感じ：
FIGHTERS  →  FIGHTERS
FEWERS    →  F-EW-ERS
              ↑   ↑
            同じ文字を
            合わせる！
```

### 🧩 アラインメントで分かること
1. **保存された部分**（よく似ている箇所）
   - タンパク質の重要な機能を担っている可能性が高い
   - 進化の過程で変わりにくかった部分

2. **変化した部分**
   - 種によって異なる働きをする可能性
   - アレルギーの強さの違いに関係するかも？

### 🔬 実験手順

#### 1️⃣ 準備
1. 配列データファイル（multi_tax.fa）をダウンロード
   - 魚類から哺乳類まで、様々な生物の配列が含まれています
   
2. [Clustal Omega](https://www.ebi.ac.uk/Tools/msa/clustalo/)にアクセス
   - 世界中の研究者が使う配列並べプログラム

#### 2️⃣ アラインメントの実行
1. "Choose File"をクリック
2. ダウンロードしたファイルを選択
3. "Submit"をクリック
4. 結果が表示されるまで約2分待つ

#### 3️⃣ 結果の見方
```
配列の例：
MKVVLVL--LAVALVGASP  (ニワトリ)
MKVVLVLLTLAVALVGASP  (ウズラ)
MKVVLAL--LAVALVGTSP  (アヒル)
**************** **   ← 記号の意味は下記
```

**記号の意味**
- `*` : 全種で同じアミノ酸（完全に保存）
- `:` : 似たアミノ酸に置き換わっている
- `.` : やや異なるアミノ酸
- `-` : アミノ酸の挿入や欠失

> 💡 **研究者のコツ**
> - アミノ酸は20種類あり、1文字で表されます
> - 保存された領域（*が多い部分）は重要な機能を持つ可能性が高い
> - 種間で違いがある部分は、機能の違いを生む可能性があります

### 📝 実験ノート
```
【よく保存されていた領域】
場所：
予想される理由：

【大きく異なっていた領域】
場所：
考えられる影響：

【アレルギーとの関連で気になった点】
観察：
仮説：
```

### 🔍 次のステップへ
アラインメントが終わったら、"Phylogenetic Tree"タブをクリック！
系統樹を見て、生物の進化とアレルギーの関係を探ってみましょう。

> ⚠️ **注意点**
> - ファイルの保存は「リンク先を別名で保存」を使います
> - 結果が表示されない場合は、ブラウザを更新してみましょう

## 👀 Step 3: 違いを探してみよう！

### 観察ポイント
1. システイン(C)の位置は同じ？
2. どこが一番違う？
3. アレルギーと関係ありそう？

### 📝 研究ノート
```
【発見したこと】
1. 
2. 
3. 

【面白いと思ったところ】
1. 
2. 

【もっと知りたいこと】
1. 
2. 
```

## 🎓 もっと深く知りたい人へ

### 🌳 分子系統樹の世界へようこそ！

#### 系統樹って何？
系統樹は、生物の「家系図」のようなものです。生物がどのように進化して今の形になったのか、その歴史を木の形で表現します。

```
         ヒト 🧍
        ╱
猿 🐒──┤
        ╲
         チンパンジー 🦍

このように、枝分かれで関係性を表現！
```

#### 分子系統樹のすごいところ
1. **進化の道筋を可視化**
   - どの生物が近い関係なのか一目で分かる
   - いつ頃分かれたのかも推測できる

2. **現代の応用例：新型コロナウイルス**
   - デルタ株やオミクロン株の関係を理解
   - ウイルスの進化を追跡
   - 新しい変異株の監視に活用

3. **遺伝子の歴史を探る**
   - 遺伝子がいつ失われたか
   - 新しい機能はいつ獲得されたか
   - 似た配列の機能を予測

### 🧪 実践！分子系統樹で分かること

#### アレルギータンパク質の例
```
ニワトリ
   ├── ウズラ
   │    └── 七面鳥
   └── アヒル
        └── ダチョウ

このように並ぶと...
近い関係の鳥ほど、アレルギー反応も似ている！？
```

#### 遺伝子の運命を追跡
例：ビタミンC合成遺伝子の物語
- 多くの動物：🟢 持っている
- ヒトを含む霊長類：❌ 失った
- いつ失った？→系統樹で分かる！

### 🔬 研究の最前線

#### 分子系統樹の活用例
1. **新薬開発**
   - 似たタンパク質の機能予測
   - 副作用の可能性の予測

2. **生物の保護**
   - 絶滅危惧種の関係性理解
   - 保護戦略の立案

3. **食品安全**
   - アレルギー反応の予測
   - 新しい食材の安全性評価

### 🎯 チャレンジ課題
自分で興味のある生物や遺伝子で系統樹を作ってみよう！

1. **調べてみたいテーマ例**
   - 好きな動物の進化の歴史
   - 食べ物に含まれるタンパク質の関係
   - 病気に関連する遺伝子の進化

2. **必要なツール**
   - MEGA（分子進化解析ソフト）
   - Clustal Omega
   - UniProt

### 📚 もっと学ぶために
- [MEGA: 分子進化解析ソフト](https://www.megasoftware.net/)
- [PDB: タンパク質の3D構造データベース](https://www.rcsb.org/)
- [Tree of Life Web Project](http://tolweb.org/)

## 🤝 困ったときは

### よくある質問
Q: 配列が見つからない！
A: IDを直接コピペしてみよう

Q: 結果の見方がわからない
A: カラフルな表示を参考に。似た色＝似たアミノ酸

### トラブル解決のコツ
1. ブラウザの更新を試す
2. 配列を再コピー
3. スタッフに相談

## 🌟 おわりに
お疲れさまです！今日はバイオインフォマティクスの研究者になった気分は味わえましたか？
この方法は、他のアレルギーの研究や、薬の開発など、様々な場面で使われています。

みなさんも、未来の研究者として、新しい発見をしてみませんか？

---
